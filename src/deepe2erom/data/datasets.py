...
class SimulationDataset:
    ...
    def __getitem__(self, idx: int) -> 'SimulationDataset':
        return SimulationDataset(
            self.states[idx:idx+1] if isinstance(idx, int) else self.states[idx],
            self.controls[idx:idx+1] if isinstance(idx, int) else self.controls[idx],
            self.states_scaled[idx:idx+1] if isinstance(idx, int) else self.states_scaled[idx],
            self.controls_scaled[idx:idx+1] if isinstance(idx, int) else self.controls_scaled[idx]
        )